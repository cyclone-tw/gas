/**
 * 50嵐飲料訂購系統 - Google Apps Script 純 GAS 版本
 * 前後端完整整合，透過 GAS 直接提供網頁服務
 */
 ...
 * 供前端呼叫 - 提交訂單
 */
function submitOrderFromClient(orderData) {
...
function getOrCreateSpreadsheet() {
  ...
  // 設定標題列
  const headers = ['訂單編號', '訂購時間', 'Email', '姓名', '部門', '飲料名稱', '甜度', '冰塊', '容量', '數量', '單價', '小計', '備註'];
...
function submitOrder(orderData) {
  try {
    ...
    // 取得登入者的 Email
    const userEmail = Session.getActiveUser().getEmail() || '未提供';
