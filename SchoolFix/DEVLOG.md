# 🛠️ 開發日誌 - GAS 踩雷記錄

> 記錄 SchoolFix 開發過程中遇到的問題與解決方案，避免未來踩雷。

---

## 🔴 錯誤 1：Date 物件無法透過 google.script.run 序列化

### 問題描述
後端函式 `getRepairRequests` 回傳的物件陣列中包含 Date 物件。在 GAS 編輯器測試正常，但透過 `google.script.run` 呼叫時，前端收到的是 `null` 或空陣列。

### 錯誤程式碼
```javascript
const request = {
  ticketId: row[0],
  reportDate: row[1],  // ❌ 這是 Date 物件！
  // ...
};
return requests; // ❌ 透過 google.script.run 回傳時會失敗
```

### 正確做法
```javascript
// ✅ 將 Date 物件轉換為 ISO 字串
const reportDate = row[1] instanceof Date 
  ? row[1].toISOString() 
  : String(row[1] || '');

const request = {
  ticketId: String(row[0] || ''),
  reportDate: reportDate,  // ✅ 字串
  // ...
};
```

### 重點
- **所有透過 google.script.run 回傳的資料必須是可序列化的純物件**
- Date、RegExp、Function 等特殊物件會導致序列化失敗
- 建議所有欄位都用 `String()` 包裝，避免 null/undefined

---

## 🔴 錯誤 2：日期比較時區問題

### 問題描述
前端傳送的日期篩選條件格式為 `2025-11-24`，後端 ISO 字串為 `2025-12-24T07:42:32.000Z`。直接用 `new Date()` 比較會有時區問題。

### 錯誤程式碼
```javascript
if (filters.startDate) {
  const startDate = new Date(filters.startDate);
  if (new Date(request.reportDate) < startDate) continue;  // ❌ 時區問題
}
```

### 正確做法
```javascript
// ✅ 使用日期字串比較，避免時區問題
if (filters.startDate && request.reportDate) {
  const reportDateStr = request.reportDate.substring(0, 10);  // YYYY-MM-DD
  const startDateStr = String(filters.startDate).replace(/\//g, '-');
  if (reportDateStr < startDateStr) continue;  // ✅ 字串比較
}
```

---

## 🔴 錯誤 3：clasp push 後未更新部署

### 問題描述
使用 `clasp push` 推送程式碼後，網頁應用程式仍顯示舊的結果。

### 原因
`clasp push` 只更新 GAS 專案的程式碼，**不會自動更新已發布的部署**。

### 正確做法
每次 `clasp push` 後：
1. 打開 GAS 編輯器
2. **部署** → **管理部署作業** → **編輯** → **新版本** → **部署**

或使用指令：
```bash
clasp deploy --description "v2"
```

---

## 🔴 錯誤 4：工作表不存在時靜默失敗

### 問題描述
當 `getSheetByName()` 回傳 `null` 時（工作表不存在），後續操作會報錯但前端只收到空結果。

### 錯誤程式碼
```javascript
function getRepairRequests(filters) {
  const sheet = ss.getSheetByName(SHEETS.REQUESTS);
  const data = sheet.getDataRange().getValues();  // ❌ sheet 可能是 null
}
```

### 正確做法
```javascript
function getRepairRequests(filters) {
  try {
    const sheet = ss.getSheetByName(SHEETS.REQUESTS);
    
    if (!sheet) {
      initializeSheets();  // ✅ 自動初始化
      return [];
    }
    
    // ...
  } catch (error) {
    Logger.log('getRepairRequests error: ' + error.message);
    return [];  // ✅ 回傳空陣列而非讓錯誤傳播
  }
}
```

---

## 🔴 錯誤 5：多個同名試算表

### 問題描述
使用 `DriveApp.getFilesByName()` 搜尋試算表時，如果 Drive 中有多個同名檔案，會取到錯誤的那個。

### 問題
- 舊專案建立了「校園修繕系統資料庫」
- 新專案也建立了「校園修繕系統資料庫」
- `getFilesByName()` 可能回傳舊的

### 解決方案
1. 刪除或重新命名舊的試算表
2. 或直接使用試算表 ID：
```javascript
function getSpreadsheet() {
  const SPREADSHEET_ID = '1KoBk0...';  // ✅ 固定 ID
  return SpreadsheetApp.openById(SPREADSHEET_ID);
}
```

---

## 🔴 錯誤 6：clasp login 授權碼過期

### 問題描述
執行 `clasp login --no-localhost` 後，複製 localhost URL 回 Cloud Shell 時出現 `invalid_grant` 錯誤。

### 原因
授權碼有效時間很短（約 1-2 分鐘）。

### 解決方案
快速完成以下步驟：
1. 點擊授權 URL
2. 登入並允許
3. **立即**複製 localhost URL
4. 貼回 Cloud Shell

---

## 🔴 錯誤 7：Report.gs 報表日期時區問題

### 問題描述
日報表「當日新增」顯示 0，但實際有當天報修的資料。

### 原因
`reportDate` 是 UTC 時間的 ISO 字串（如 `2025-12-24T07:42:32.000Z`），但報表用本地時間建立的 `startOfDay`/`endOfDay` 比較，導致時區不一致。

### 錯誤程式碼
```javascript
const todayRequests = allRequests.filter(r => {
  const d = new Date(r.reportDate);  // ❌ UTC 時間
  return d >= startOfDay && d <= endOfDay;  // ❌ 本地時間比較
});
```

### 正確做法
```javascript
// ✅ 使用 Utilities.formatDate 統一轉換為台北時間
const targetDateStr = Utilities.formatDate(targetDate, 'Asia/Taipei', 'yyyy-MM-dd');

const todayRequests = allRequests.filter(r => {
  if (!r || !r.reportDate) return false;
  const utcDate = new Date(r.reportDate);
  const reportDateStr = Utilities.formatDate(utcDate, 'Asia/Taipei', 'yyyy-MM-dd');
  return reportDateStr === targetDateStr;  // ✅ 字串比較
});
```

### 重點
- **GAS 中使用 `Utilities.formatDate(date, 'Asia/Taipei', 'yyyy-MM-dd')` 統一時區**
- 不要用 JavaScript 的 `setHours()` + Date 比較

---

## 🟢 GAS 部署最佳實踐

### 1. 資料序列化
- 所有回傳前端的資料都用純物件（字串、數字、布林值）
- Date 轉 `toISOString()`
- 用 `String()` 包裝可能是 null 的欄位

### 2. 錯誤處理
- 所有 API 函式用 try-catch 包裝
- 回傳 `{ success: true/false, error: message }`
- 使用 `Logger.log()` 記錄錯誤

### 3. 部署流程
```bash
# 開發
git pull
# 修改程式碼
clasp push

# 更新部署（GAS 編輯器）
部署 → 管理部署作業 → 編輯 → 新版本 → 部署

# 或命令列
clasp deploy
```

### 4. 除錯技巧
- 在 GAS 編輯器執行函式查看 Logger 輸出
- 前端用 `alert()` 或 `console.log()` 顯示 API 回傳值
- 檢查試算表是否有資料
- 確認部署版本是最新

### 5. 日期處理（重要！）
- **儲存**：使用 `new Date()` 原生 Date 物件
- **回傳前端**：轉為 ISO 字串 `toISOString()`
- **比較日期**：使用 `Utilities.formatDate(date, 'Asia/Taipei', 'yyyy-MM-dd')` 字串比較

---

## 📅 開發時間軸

| 日期 | 事件 |
|-----|------|
| 2025-12-23 | 專案初始化、基本框架完成 |
| 2025-12-24 | 發現資料持久化問題 |
| 2025-12-24 | 確認問題：Date 物件序列化失敗 |
| 2025-12-24 | 修復：轉換所有 Date 為字串 |
| 2025-12-24 | 修復：日期比較邏輯 |
| 2025-12-24 | Cloud Shell 部署成功 ✅ |
| 2025-12-24 | 修復：Report.gs 時區問題 ✅ |
| 2025-12-24 | 設定 GitHub Actions 自動部署 ✅ |
| 2025-12-24 | 修正 .clasp.json 專案 ID ✅ |
| 2025-12-24 | 樣式更新為 Flutter 風格 ✅ |
| 2025-12-24 | 網站品牌更新：國姓國小報修系統 ✅ |
